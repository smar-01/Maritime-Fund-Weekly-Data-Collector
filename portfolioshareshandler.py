...
def load_portfolio_shares_total(portfolio_tickets_total):
    """
    ...
    all_dates = set()
    for stock, events in portfolio_tickets_total.items():
        for event in events:
    ...
    for stock, events in portfolio_tickets_total.items():
        ...
        events = sorted(portfolio_tickets_total[stock], key=lambda x: datetime.strptime(x['event_date'], '%m/%d/%Y %H:%M:%S'))
